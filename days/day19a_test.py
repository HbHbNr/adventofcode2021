...
class TestDay19a(unittest.TestCase):
    ...
    def testVectorRotateAxes(self) -> None:
        coords = Coords(1, 2, 3)
        assert str(coords) == '(1/2/3)'
        coords.rotateAxes()
        assert str(coords) == '(3/1/2)'
        coords.rotateAxes()
        assert str(coords) == '(2/3/1)'
        coords.rotateAxes()
        assert str(coords) == '(1/2/3)'
    ...
    def testDistanceTurnLongestToPositiveX(self):
        ...
        assert str(distance) == '(1/2/-3)'
        distance.turnLongestToPositiveX()
        assert str(distance) == '(3/1/-2)'
